...
* Muuda ansible kataloogis ära `ansible_ssh_host` väärtus failis `inventory.ini`. `ansible_ssh_host` on remote provisioneeritava ip aadress (mitte selle masina aadress, kus te ansiblet installeerisite), EC2 serveri puhul leiad selle IP oma aws konsoolist (aws konsooli all näitab ec2 private  ip ja ec2 public ip, kasutage public). Ehk siis see on selle masina IP, kus programm tööle hakkab.
